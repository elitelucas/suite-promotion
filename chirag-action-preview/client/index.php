...
print_r($info);
echo '</pre>';
echo 'URL === '.$info['media']->url;
echo 'chirag allPlugins<br>';
echo '<pre>';
print_r($allPlugins);
echo '</pre>';*/
...
<!-- Spin Wheel 
<link rel="stylesheet" href="<?php echo PATH_ROOT ?>/plugins/spin-wheel/css/sweetalert2.min.css"> 
<link rel="stylesheet" href="<?php echo PATH_ROOT ?>/plugins/spin-wheel/css/superwheel.min.css"> 
<link rel="stylesheet" href="<?php echo PATH_ROOT ?>/plugins/spin-wheel/css/style.css"> 
...
<script src="<?php echo PATH_ROOT ?>/plugins/spin-wheel/js/jquery.superwheel.min.js"></script>
<script src="<?php echo PATH_ROOT ?>/plugins/spin-wheel/js/sweetalert2.min.js"></script> 
		...
		<div id="accordion">
				...
				<div id="collapse3" class="panel-collapse collapse">
					<div class="card-body">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control headlineinput" name="caption" id="caption" maxlength="140" aria-describedby="helpIdCaption" placeholder="Enter Caption">
						</div>						
			...
			<div class="card-custom card card-default">
				...
				<div id="collapse4" class="panel-collapse collapse">
					...
					<div class="form-group">
						...
						<select class="form-control" name="action" id="action" required>
							...
							if ($data && count($data)>0) {
								foreach($data as $idx => $plugins):
									?>
									<option value="<?php echo $plugins['filename'];?>"><?php echo $plugins['actionName'];?></option>
								<?php
								endforeach;
							}
							?>
						...
						if ($data && count($data)>0) {
							foreach($data as $idx => $plugins):
								...
								?>
								<input type="hidden"
									   id="plugin-<?php echo $plugins['filename']; ?>"
									   name="plugin-<?php echo $plugins['filename']; ?>"
									   value="<?php echo $plugins['type'];?>" />
							<?php
							endforeach;
						}
						?>
					...
					<button type="button" id="addActionBtn" class="btn bg-gradient-success">Add Action</button>
...
<div class="login-box">
  ...
  <div class="card">
	...
	<p>
	<?php 
		if(isset($info['media']->url) && $info['media']->url != '')
		{
			echo '<img style="margin-top:12px" id="banner_image" width="100%" src="'.$info['media']->url.'"/>';
		}
		else
		{
			echo '<img style="margin-top:12px" id="banner_image" width="100%" src="<?php echo PATH_ROOT ?>/images/banner.jpg"/>';	
		}
	?>
	</p>
	  <?php 
	  	if($pluginTitle != '')
	  		echo '<div class="login-logo headlinebox"><b>'.$pluginTitle.'</b></div>';
	  	else
	  		echo '<div class="login-logo headlinebox"><b>Your headline here</b></div>';
	  	?>
...
<div class="card-body">
                ...
                <div id="accordion" class="addedActionPrv">
				...
				<div class="alert alert-danger alert-dismissible">
				  ...
				  <!---------------------------------------------------- ACTION ---------------------------------------------------->				
				  <?php
				  $allSocials = (array)$pluginData['data'];

				  if($allSocials && count($allSocials) > 0)
				  {
				  	//for($i=0; $i<$allPlugins; $i++)
				   ...
				   echo '</pre>';*/
						...
						if ($delayTime < 1) {
				  		...
				  		if($type == 'visit-and-share' && $network =='facebook') {
				  	?>
				  	...
				  	<?php	
				  		}	// if($type == 'visit-and-share' && $network =='facebook') OVER
				  		else if($type == 'share-and-visit' && $network =='facebook') {
				  	?>
				  		<div class="card card-default">
...
$("#create-client-plugin").submit(function(e) {
	...
	$.ajax({
		...
		success: function(data)
		{
			...
			alert('Created Plugin successfully 11111.'); // show response from the php script.
...
$('#addActionBtn').click(function() {
	...
	var visitAct = $( "#visitAction" ).val();
	var shareAction = $( "#shareAction" ).val();
	var numpoint = $( "#numpoint" ).val();
	///
	
	/*<a id="action-'+action+'-'+selectedNetwork+'" href="javascript:void(0);"			   style="color: #fff; -webkit-transition: width , height;transition: width , height;-webkit-transition-delay: <?php echo $delayTime?>s;transition-delay: <?php echo $delayTime?>s;" class="btn btn-block btn-social btn-'+selectedNetwork+'" data-toggle="collapse" aria-expanded="false" aria-controls="FBLike'+action+'">'+
				'<i class="fab fa-'+selectedNetwork+' fa-2x"></i> '+shareAction+'</a>*/

var addedActionHTML = '<div class="card card-default">'+

	'<a class="info-box" data-toggle="collapse" data-parent="#accordion" href="#collapse'+ randomNum +'">'+
		'<span id="'+selectedNetwork+'" class="info-box-icon bg-info"><i class="fab fa-'+selectedNetwork+'"></i></span>'+

		'<div class="info-box-content">'+
			
			'<span class="info-box-text">'+visitAct+'</span>'+

		'</div>'+
		
		'<span id="action-point-'+action+'-'+selectedNetwork+'" class="badge badge-danger float-right">+'+numpoint+'</span>'+
		
	'</a>'+	
	'<div id="collapse'+randomNum+'" class="panel-collapse collapse in">'+
		'<div class="card-body">'+
			'<a id="action-'+action+'-'+selectedNetwork+'" href="javascript:void(0);"			   style="color: #fff; -webkit-transition: width , height;transition: width , height;-webkit-transition-delay: <?php echo $delayTime?>s;transition-delay: <?php echo $delayTime?>s;" class="btn btn-block btn-social btn-'+selectedNetwork+'" data-toggle="collapse" aria-expanded="false" aria-controls="FBLike'+action+'">'+
				'<i class="fab fa-'+selectedNetwork+' fa-2x"></i> '+shareAction+'</a>'+

			'<div style="display: none" class="collapse text-center" id="action-content-'+action+'-'+selectedNetwork+'">'+

				'<p id="message-description-'+action+'"><i class="fas fa-check"></i> Thanks for visiting, share to add points!</p>'+

				'<p id="share-button"><a href="javascript:void(0);"  style="color: #fff;" class="btn btn-block btn-social btn-'+selectedNetwork+' sharefb'+action+'-'+selectedNetwork+'">'+
						'<i class="fab fa-'+selectedNetwork+' fa-2x"></i> '+action+'</a></p>'+
			'</div>'+
...
//var addedActionHTML = '<div class="card card-default">Chirag Shah HTML added </div>';
//$('.addedActionPrv').html(addedActionHTML);
$(addedActionHTML).insertAfter('.addedActionPrv .alert-dismissible');
//$('.addedActionPrv .alert-dismissible').html(addedActionHTML);
$('select.select2').append('<option value="'+shareAction+'" data-select2-id="'+randomNum+'">'+shareAction+'-'+selectedNetwork+'</option>');
// var newOption = new Option(action+'-'+selectedNetwork, action, false, false);
// $('.select2').append(newOption).trigger('change');
$('.select2').val(shareAction);
...
<script>
    ...
    $(function () {
        ...
        $("#brand_pic_url").change(function() {
	        $('#banner_image').attr('src', $("#brand_pic_url").val());
	    });
    ...
    function readURL(input) {
    	alert('heere  readURL'+input.files);
